...
func sample(
  ...
  mcpServerId: String, mcpToolId: String
) async throws {
  let response = try await client.getMcpTool(
    request: GetMcpToolRequest()
      .with {
        $0.name =
          "projects/\(projectId)/locations/\(locationId)/apiNamespaces/\(apiNamespaceId)/mcpServers/\(mcpServerId)/mcpTools/\(mcpToolId)"
      }
  )
  ...
  static func main() async throws {
    do {
      ...
      try await sample(
        ...
        apiNamespaceId: "[placeholder]", mcpServerId: "[placeholder]", mcpToolId: "[placeholder]")
    } catch {
      print("Error: \(error)")
    }
  }
