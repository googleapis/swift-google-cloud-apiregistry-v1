...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-apiregistry-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudApiRegistryV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudApiRegistryV1 <target-name> --package swift-google-cloud-apiregistry-v1
```
